...
while read station_name
  do
    ...
    echo "`find . -name  annotations.h5 |grep 160726_ `"
	find -L . -name annotations.h5 | grep -v "chunked_results" |grep $1_  >> $GPU_TEST_HOME/log/TodayAnnotaions_$1.txt
    echo ""
    echo "Done with seaching annotations.f5 from  machine: `echo $station_name`!"
    echo "##############################################################"
    #fi
    #sleep 1
done
