...
##############################################################################
#
# Script Name: installer_9.0.1.2.sh
# Script Duty: Installs ESB, EDS, EAS, and SVP 9012 builds. 
# Author: Tuan M. Mai
# Date: 06/28/2006
#
##############################################################################
...
INFILE_DIR="$HOME/bin/infiles/9.0"

#### install Essbase ####

if [ "$INSTALL_PRODUCT" = "ESB" ] ; then
	
	ESB_BUILD=$BUILD_ROOT/essbase/builds/9.0/9.0.1.2/$INSTALL_BUILD/cd/$INSTALL_OSP
...
if [ "$INSTALL_PRODUCT" = "EAS" ] ; then
	EAS_BUILD=$BUILD_ROOT/EAS/builds/9.0.1.2/$INSTALL_BUILD/install/$INSTALL_OSP	
...
if [[ "$INSTALL_PRODUCT" = "EDS" ]] ; then
	EDS_BUILD=$BUILD_ROOT/eesdev/eds/builds/9.0.1.2/$INSTALL_BUILD/install/$INSTALL_OSP
...
if [[ "$INSTALL_PRODUCT" = "SVP" ]] ; then
	SVP_BUILD=$BUILD_ROOT/eesdev/provider/builds/9.0.1.2/$INSTALL_BUILD/install/$INSTALL_OSP
