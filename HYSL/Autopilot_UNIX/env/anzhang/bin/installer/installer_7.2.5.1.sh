...
##############################################################################
#
# Script Name: installer_7.2.5.1.sh
# Script Duty: Installs ESB, EDS, EAS, and SVP 7.2.5.1 builds. 
# Author: Tuan M. Mai
# Date: 06/28/2006
# Note: Only EAS is modified to install EAS 7.2.5.1, since the comparison is only
#       between EAS 7.2.5.0 and EAS 7.2.5.1
##############################################################################
...
if [ "$INSTALL_PRODUCT" = "ESB" ] ; then
	...
	ESB_BUILD=$BUILD_ROOT/essbase/builds/7.2.5.1/$INSTALL_BUILD/cd/$INSTALL_OSP
...
if [ "$INSTALL_PRODUCT" = "EAS" ] ; then
	EAS_BUILD=$BUILD_ROOT/EAS/builds/7.2.5.1/$INSTALL_BUILD/install/$INSTALL_OSP	
...
if [[ "$INSTALL_PRODUCT" = "EDS" ]] ; then
	EDS_BUILD=$BUILD_ROOT/eesdev/eds/builds/7.2.5.1/$INSTALL_BUILD/install/$INSTALL_OSP
...
if [[ "$INSTALL_PRODUCT" = "SVP" ]] ; then
	SVP_BUILD=$BUILD_ROOT/eesdev/provider/builds/7.2.5.1/$INSTALL_BUILD/install/$INSTALL_OSP
