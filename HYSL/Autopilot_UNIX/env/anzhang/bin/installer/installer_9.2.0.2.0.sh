...
##############################################################################
#
# Script Name: installer_9.2.0.1.0.sh
# Script Duty: Installs ESB, EDS, EAS, and SVP 9.2.0.1.0 builds. 
# Author: Tuan M. Mai
# Date: 05/05/2006
#
##############################################################################
...
if [ "$INSTALL_PRODUCT" = "ESB" ] ; then
	...
	if [ "$INSTALL_OSP" = "solaris64" ] ; then
		INSTALL_OSP="solaris"
	fi

	if [ "$INSTALL_OSP" = "aix64" ] ; then
		INSTALL_OSP="aix"
	fi
			
	ESB_BUILD=$BUILD_ROOT/essbase/builds/9.2/9.2.0.2.0/$INSTALL_BUILD/cd/$INSTALL_OSP
...
if [ "$INSTALL_PRODUCT" = "EAS" ] ; then
	EAS_BUILD=$BUILD_ROOT/EAS/builds/9.2.0.2.0/$INSTALL_BUILD/install/$INSTALL_OSP	
...
if [[ "$INSTALL_PRODUCT" = "EDS" ]] ; then
	EDS_BUILD=$BUILD_ROOT/eesdev/eds/builds/9.2.0.2.0/$INSTALL_BUILD/install/$INSTALL_OSP
...
if [[ "$INSTALL_PRODUCT" = "SVP" ]] ; then
	SVP_BUILD=$BUILD_ROOT/eesdev/provider/builds/9.2.0.2.0/$INSTALL_BUILD/install/$INSTALL_OSP
