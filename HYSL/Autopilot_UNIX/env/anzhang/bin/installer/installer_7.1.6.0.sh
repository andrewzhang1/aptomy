...
##############################################################################
#
# Script Name: installer_7.1.6.0.sh
...
if [ "$INSTALL_PRODUCT" = "ESB" ] ; then
	...
	ESB_BUILD=$BUILD_ROOT/essbase/builds/7.1/7.1.6.0/$INSTALL_BUILD/cd/$INSTALL_OSP
	...
	if [ "$INSTALL_OSP" = "win32" -o "$INSTALL_OSP" = "win64" ] ; then
		...
		if [ "$INSTALL_OSP" = "linux" ] ; then
			$ESB_BUILD/server/setup.sh -console < $INFILE_DIR/esbinstsq_linux.txt
			$ESB_BUILD/client/setup.sh -console < $INFILE_DIR/esbclient_linux.txt
		else	
...
if [ "$INSTALL_PRODUCT" = "EAS" ] ; then
	EAS_BUILD=$BUILD_ROOT/EAS/builds/7.1.6.0/$INSTALL_BUILD/install/$INSTALL_OSP	
...
if [[ "$INSTALL_PRODUCT" = "EDS" ]] ; then
	EDS_BUILD=$BUILD_ROOT/eesdev/eds/builds/7.1.6.0/$INSTALL_BUILD/install/$INSTALL_OSP
...
if [[ "$INSTALL_PRODUCT" = "SVP" ]] ; then
	SVP_BUILD=$BUILD_ROOT/eesdev/provider/builds/7.1.6.0/$INSTALL_BUILD/install/$INSTALL_OSP
