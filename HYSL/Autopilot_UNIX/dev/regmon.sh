...
# 2013/05/30	YKono	Ignore application alive check with I18N test.
# 2013/08/07	YKono	BUG 17278499 - AUTOPILOT KILLS I18N APP WITH JAPANESE NAME 
# 2013/11/14	YKono	Bug 14733329 - NEED TO GENERATE DIF WHEN AP KILLS HANGUP PROCESSES
# 2014/03/13	YKono	Bug 17814337 - WHEN KILL THE HANGING UP PROCESS, PUT PSTACK OUTPUT INTO ARCHIVE AND E-MAIL 
#			Bug 18383946 - REGRESSION MONITOR SHOULD RE-TRY THE AGENT/SERVER HANGING-UP TEST

trap 'echo "ctrl+C (regmon.sh)";msg \"ctl+C exit\"' 2
. apinc.sh
...
killnotify()
{
	...
	cat $killrec >> "$notify_mail"
	send_email "$notify_mail"
	cp $killrec ${sxr_work}/{$crr_test%.*}_killed.dif
}
...
pstck=`which pstack 2> /dev/null`
[ -z "$pstck" -o "${pstck#no}" != "$pstck" ] && unset pstck
echo "pstck=[$pstck]"
echo "rglog=$rglog"
...
while read line; do
	...
	case $var in
		SXR_WORK)
			export sxr_work="$(eval echo $val)"
			;;
		SXR_USER)
			export SXR_USER="$(eval echo $val)"
			;;
		SXR_PASSWORD)
			export SXR_PASSWORD="$(eval echo $val)"
			;;
		SXR_DBHOST)
			export SXR_DBHOST="$(eval echo $val)"
			;;
		HYPERION_HOME)
			export hyperion_home="$(eval echo $val)"
			;;
	esac
done < "$envfile"
tarbase=${tartest%%.*}
target_sog=$sxr_work/${tarbase}.sog
target_sta=$sxr_work/${tarbase}.sta
[ "${tarbase#i18n_}" != "$tarbase" -o "${tarbase}" = "xprobmain" ] && i18nflag=true || unset i18nflag
...
msg "SXR_DBHOST    =$SXR_DBHOST"
msg "SXR_USER      =$SXR_USER"
msg "SXR_PASSWORD  =$SXR_PASSWORD"
msg "hyperion_home =$hyperion_home"
msg "#######################################################################"
...
while [ "$loop" = "true" ]; do
	...
	case $act in
		...
		hang_test)
			...
			apps=`sv_crrapp.sh`
			msg "Apps:" $apps
			if [ "$i18nflag" != "true" -a -n "$apps" ]; then
				act=wait
				msg "Doing app hang test."
				for app in $apps; do
					# msg "Test $app"
					sts=`sv_hang_test.sh $ver -a $app`
					msg "sv_hang_test.sh $ver -a $app = $sts"
					if [ "$sts" = "agent" -o "$sts" = "login" ]; then
						...
						apid=${app#*:}
						app=${app%:*}
						psts=`ps -p ${apid} 2> /dev/null | grep -v PID`
						if [ -n "$psts" ]; then
							msg "<$app> - hang."
							[ -n "$pstck" ] \
								&& pstack $apid > \
								"$sxr_work/killedrec-`date +%m%d%y-%H%M%S`-app-${app}.pstack"
							[ -f "$hyperion_home/logs/essbase/app/$app/$app.log" ] \
								&& cp "$hyperion_home/logs/essbase/app/$app.log" \
									"$sxr_work/killedrec-`date +%m%d%y-%H%M%S`-app-${app}.app.log"
							[ -f "$hyperion_home/logs/essbase/Essbase.log" ] \
								&& cp "$hyperion_home/logs/essbase/Essbase.log" \
								"$sxr_work/killedrec-`date +%m%d%y-%H%M%S`-app-${app}.essbase.log"
							sleep 30
							kill_essprocs.sh $app | msg
							echo "`date +%D_%T`:$crrscr/<$app>" >> "$killrec"
							killnotify "APP:$app" $crrscr "Killed application:$app"
							cnt=`grep "^../../.._..:..:..:$crrscr/<$app>\$" "$killrec" \
								2> /dev/null | wc -l`
							let cnt=cnt
		...
		agent_hang_test|agent_hang_proc)
			...
			act=wait
			if [ "$sts" != "false" ]; then
				apid=`psu -ess | egrep -i essbase | head -1`
				apid=`echo $apid | awk '{print $1}'`
				psts=`ps -p $apid 2> /dev/null| grep -v PID`
				if [ -n "$psts" ]; then
					msg "- Failed to login."
					[ -n "$pstck" ] \
						&& pstack $apid > \
						"$sxr_work/killedrec-`date +%m%d%y-%H%M%S`-agent.pstack"
					[ -f "$hyperion_home/logs/essbase/Essbase.log" ] \
						&& cp "$hyperion_home/logs/essbase/Essbase.log" \
						"$sxr_work/killedrec-`date +%m%d%y-%H%M%S`-agent.essbase.log"
					sleep 30
					kill_essprocs.sh -agtsvr | msg
					...
					killnotify "AGENT" $crrscr \
						"Found agent hang-up and killed agent." \
						"Agent killed count=$cnt" "Next act=$act"
				fi
			fi
			;;
		...
		client_timeout)
			...
			psu -ess | egrep -i "esscmd|essmsh" | while read pid cli; do
				psts=`ps -p $pid 2> /dev/null | grep -v PID`
				if [ -n "$psts" ]; then
					klltrg=`psu | grep ^$pid`
					[ -n "$pstck" ] \
						&& pstack $pid > \
						"$sxr_work/killedrec-`date +%m%d%y-%H%M%S`-client-${cli}.pstack"
					...
					killnotify "CLI:$cli" $crrscr "killtarget=$pid:$cli $klltrg" \
						"elapsed time=$etime($AP_CLIENT_TIMEOUT)" "Killed count=$cnt"
					[ $cnt -ge 3 ] && break
				fi
			done
			cnt=`grep "^../../.._..:..:..:$crrscr/\[client\]" "$killrec" 2> /dev/null | wc -l`
			...
			while [ 1 ]; do
				...
				if [ $cnt -lt 3 ]; then
					...
					if [ -n "$scrpid" ]; then
						...
						[ -n "$pstck" ] \
							&& pstack $killpid > \
							"$sxr_work/killedrec-`date +%m%d%y-%H%M%S`-scr-${crrscr##*/}.pstack"
						sleep 5
		...
		wait)	# Waiting loop for the next check (AP_REGMON_INTERVAL)
			...
			while [ "$crrtime" -lt "$inttime" ]; do
				...
				(
				echo "$addrlist"
				echo "Autopilot Notification (DiffCnt over) - $crr_ver $crr_bld - $crr_host:$crr_plat $crr_test"
				echo "Total diff count exceeded on following test."
				echo "  Machine         = $crr_host:$crr_plat"
				echo "  Login User      = $LOGNAME"
				echo "  Version:Build   = $crr_verbld"
				echo "  Test Script     = $crr_test"
				echo "  Sub Script      = $subs"
				echo "  Suc/Dif count   = $succ/$difc (threshold:$diffcnt)"
				echo "  Data/Time       = `date +%D_%T`"
				) > $notify_mail
							send_email "$notify_mail"
