...
# 04/23/2013	YKono	Add workaround for windows
# 06/07/2013	YKono	Bug 16908191 - AUTOPILOT SUPPORT REPOSITORY CREATION SERVICE AS DEAMON.
# 07/19/2013	YKono	Add README.htm contents to the bipatch_version.txt
# 07/29/2013	YKono	Bug 17236360 - BI PATCH SUPPORT MULTIPLE ZIP INSTALLATION.
# 08/16/2013	YKono	Bug 17322816 - BIINST.SH FAILED TO CREATE REPOSITORY FOR BISHIPHOME_11.1.1.7.0BI-FAREL8-BP
# 08/16/2013	YKono	Check patch folder is valid or not (check -f etc/config/inventory.xml)
# 08/21/2013	YKono	Run BISHIPHOME installer in background on win32. And check
# 			the WerFault.exe launch. If find WerFault.exe, kill that process.
# 01/16/2014	YKono	Bug 17715224 - ADD OPATCH ID TO THE RTF FILE 
...
while [ $# -ne 0 ]; do
	case $1 in
		...
		*:*:*:*)	# Predefined-Repository: Repository constr Default prefix
		...
		-essver)
			if [ $# -lt 2 ]; then
				echo "${me##*/}:\"$1\" option need 2nd parameter as an expected Essbase version."
				exit 1
			fi
			shift
			essver=$1
...
if [ "$rcu" = "true" ]; then
	...
	# - Get version Label from $biptr/Labels.txt
	# Integration Label:  BISHIPHOME_11.1.1.7.0_HPUX.IA64_130306.1901
	if [ "$dbg" = "true" ]; then
		echo "### Create BI repository database."
		echo "### BUG 16908191 - AUTOPILOT SUPPORT REPOSITORY CREATION SERVICE AS DEAMON."
	fi
	lbl=`head -1 $biptr/bishiphome/Labels.txt 2> /dev/null`
	lbl=${lbl##* }
	_biver=${lbl#BISHIPHOME_}
	_biver=${_biver%%_*}
	_biver=${_biver%.*}
	[ "$dbg" = "true" ] && echo "### Label version=$_biver($lbl)"
	[ "$_biver" = "11.1.1.6" ] && _biusr="RCU6:${prf}" || _biusr="RCU7:${prf}"
	rm -rf $HOME/.dbreq.$$.out
	[ "$dbg" = "true" ] && echo "### Execute \"bireq.sh -n $_biusr\" now..."
	dbreq.sh -n $_biusr > $HOME/.dbreq.$$.out 2>&1
	_sts=$?
	if [ "$_sts" -eq 0 ]; then
		_sts=`cat $HOME/.dbreq.$$.out 2> /dev/null | grep "^sts=" | tr -d '\r'`
		_sts=${_sts#sts=}
		if [ "$dbg" = "true" ]; then
			echo "### dbreq.sh -b $_biusr return $_sts."
			cat $HOME/.dbreq.$$.out 2> /dev/null | while read _line; do echo "### > $_line"; done
		fi
		if [ "$_sts" = "0" ]; then
			bi_db=`cat $HOME/.dbreq.$$.out 2> /dev/null | grep "^constr=" | tr -d '\r'`
			bi_db=${bi_db#constr=}
			mds_db=$bi_db
			bi=`cat $HOME/.dbreq.$$.out 2> /dev/null | grep "^bi=" | tr -d '\r'`
			bi=${bi#bi=}
			mds=`cat $HOME/.dbreq.$$.out 2> /dev/null | grep "^mds=" | tr -d '\r'`
			mds=${mds#mds=}
			bi_pwd=`cat $HOME/.dbreq.$$.out 2> /dev/null | grep "^pwd=" | tr -d '\r'`
			bi_pwd=${bi_pwd#pwd=}
			mds_pwd=$bi_pwd
			echo "### dbreq.sh -n $_biusr return $_sts"
			echo "### - bi_db=$bi_db"
			echo "### - mds_db=$mds_db"
			echo "### - bi=$bi"
			echo "### - bi_pwd=$bi_pwd"
...
if [ "$rcu" = "true" ]; then
	...
	echo "### biinst.sh:`date +%D_%T` Skip RCU."
fi
if [ $sts -eq 0 ]; then
	...
	if [ "${thisplat#win}" != "$thisplat" ]; then
		...
		# Delete inventry log files
		rm -rf "$prgdir/Oracle/Inventory/logs" 2> /dev/null
		(
			_cmdl="$biptr/$prg -silent -response $rsp -waitforcompletion"
			if [ "$dbg" = "true" ]; then
				_xmlf="${mw_home}/user_projects"
				_xmlf="${_xmlf}/domains/bifoundation_domain"
				_xmlf="${_xmlf}/config/fmwconfig/jps-config-jse.xml"
			fi
			[ "$thisplat" = "win32" ] && installer_in_bg=true || installer_in_bg=false
			unset adm adm_pwd alllck alltsk ampar amsal APBIN APCC aphis apsal \
			  AP_BISHIPHOME AP_CRRTSK AP_DEFAULT_CCC AP_DEFVAR \
			  AP_DEF_ACCPATH AP_DEF_BASPATH AP_DEF_BINPATH AP_DEF_DATPATH \
			  AP_DEF_DIFFOUT AP_DEF_DMAPP AP_DEF_EMAILLIST AP_DEF_ENVPATH \
			  AP_DEF_ENVPATH_TEMPLATE AP_DEF_FLGPATH AP_DEF_IGNORESH \
			  AP_DEF_LOGPATH AP_DEF_MONPATH AP_DEF_QTSK AP_DEF_RESLOC \
			  AP_DEF_RESPATH AP_DEF_RSPPATH AP_DEF_SCRIPTLIST AP_DEF_SECPATH \
			  AP_DEF_SHABBR AP_DEF_SNAPSHOT_BASEPATH AP_DEF_STSPATH \
			  AP_DEF_SXRAGENTPORTLIST AP_DEF_TMPPATH AP_DEF_TSKPATH \
			  AP_DEF_VIEWPATH AP_NOPLAT AP_PLATGROUP AP_RTDIFFILE \
			  AP_RTENVFILE AP_RTSOGFILE AP_RTSTAFILE AP_RTFILE AP_SECMODE \
			  AP_TSKDONE AP_TSKQUE ARCH _OPTION AP_BICONSTR \
			  AP_DEF_AGENTPORTLIST AP_DEF_INSTERRSTS AP_DEF_LICPATH AP_NOPLAT \
			  AUTOPILOT BUILD_ROOT PROD_ROOT HIT_ROOT ESSLANG HYPERION_HOME VIEW_PATH \
			  bi bireplist birepmethod bi_db bi_dbtyp bi_pwd brnch \
			  cnt conpwd conroll constr conusr crrdir dbg \
			  invPtr lbl lib mds mds_db mds_dbtyp mds_pwd me \
			  mytsk mywork my_crr_base node nval one opt orgpar org_mwhome \
			  pathsep plat platlck platrlck plattsk \
			  prf rcu rcuprg rcuptr sec secmode src stgfile str svusr sts \
			  SXR_PASSWORD SXR_USER t prgdir H \
			  thishost thiskernel thisnode thisplat thissvusr this_flag_file \
			  this_status_file tsklck val vdf ve wrtcrr \
			  _apbin _aporgps1 _apps1 _crrval _rsp _tmp _val \
			  PWD OLDPWD FrameworkDir FrameworkSDKDir FrameworkVersion \
			  ICPP_COMPILER11 ICPP_COMPILER12 CommonProgramFiles CommonProgramW6432 \
			  INCLUDE include INETSDK ININTEL_COMPILER80E INTEL_LINCESE_FILE \
			  LIB LIBPATH MSSdk Mstools \
			  VCINSTALLDIR VS100COMNTOOLS VS80COMNTOOLS VS90COMNTOOLS \
			  CSEDEFLOGDIR VSINSTALLDIR
			unset biptr prg rsp mw_home
			[ "$dbg" = "true" ] && set | tee $HOME/vars.txt
			if [ "$installer_in_bg" = "false" ]; then
				$_cmdl
				sts=$?
			else
				$_cmdl &
				bipid=$!
				prgdir=`registry -p -r -k \
				  "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion" \
				  -n "ProgramFilesDir" 2> /dev/null`
				echo "# BI pid=$bipid, prgdir=$prgdir"
				cl=0; logf=
				while [ -n "`ps -p $bipid 2> /dev/null | grep -v PID`" ]; do
					...
					werfault=`psu | grep WerFault.exe | grep -v grep`
					[ -n "$werfault" ] && kill -9 ${werfault%% *} > /dev/null 2>&1
					wermgr=`psu | grep wermgr.exe | grep -v grep`
					[ -n "$wermgr" ] && kill -9 ${wermgr%% *} > /dev/null 2>&1
				done
				sts=0
			fi
			if [ "$dbg" = "true" ]; then
		...
		sts=$?
		# Work around
		jpsxml="${mw_home}/user_projects/domains"
		jpsxml="${jpsxml}/bifoundation_domain/config"
		jpsxml="${jpsxml}/fmwconfig/jps-config-jse.xml"
		if [ -f "$jpsxml" ]; then
			rm -f ${jpsxml}.ap.bak
			mv $jpsxml ${jpsxml}.ap.bak
			cat ${jpsxml}.ap.bak | sed -e \
				"s/ou=myrealm,dc=.*$/ou=myrealm,dc=bifoundation_domain\<\/value\>/g" \
				> $jpsxml
			workaround=$mw_home/Oracle_BI1/workaround.txt
			echo "biinst.sh:jsp-config-jse.xml replacing problem.(Chinta)" >> $workaround
			echo "# jsp-config-jse.xml under <MH>/user_projects/domains/bifoundation_domain/config/fmwconfig" >> $workaround
			echo "# sed -e s/ou=myrealm,dc=.*$/ou=myrealm,dc=bifoundation_domain<value>/g" >> $workaround
		fi
	else
...
if [ $sts -ne 0 ]; then
	...
	rm -rf $HYPERION_HOME/bipatch_version.txt
	# Apply workaround and Check patch folder and apply
	(
		# Get Essbase version
		[ -z "$essver" ] && essver=`ess_ver $HYPERION_HOME/products/version-xml/essbase_version.xml`
		. se.sh ${essver%:*} -${secmode} 
		# . se.sh ${essver%:*} -${secmode} > /dev/null 2>&1
		( # Workaround
			if [ "`uname`" = "HP-UX" ]; then
				for patchfile in \
					$HYPERION_HOME/clients/epm/Essbase/EssbaseRTC/bin/libglobalc.sl \
					$ESSBASEPATH/bin/libglobalc.sl; do
				globalc=`ls -l $patchfile 2> /dev/null`
				attr=${globalc%% *}
				if [ "${attr#*w}" = "$attr" ]; then
					(
					echo "biinst.sh(hpux64/BI-11.1.1.7.0) - libglobalc.sl doesn't have a write permission."
					echo "# $patchfile"
					echo "# > $globalc"
					echo "# chmod a+w ${patchfile}"
					) >> $HYPERION_HOME/workaround.txt
					chmod a+w $patchfile
				fi
				done
			fi
		)
		patches=`chk_para.sh bipatchimg "$_OPTION"`
		if [ -z "$patches" ]; then
			patches=`chk_para.sh bipimg "$_OPTION"`
		fi
		if [ -z "$patches" ]; then
			patches=`ls -l $biptr | grep ^d | grep -v bishiphome | grep -v rcuHome | \
				while read d1 d2 d3 d4 d5 d6 d7 d8 fld; do 
					[ -f "$biptr/$fld/etc/config/inventory" -o \
					  -f "$biptr/$fld/etc/config/inventory.xml" ] \
						&& echo "$biptr/$fld"
				done`
		fi
		cd $HYPERION_HOME/OPatch
		for fld in $patches; do
			if [ -d "$fld/etc/config" ]; then
				echo "### Found $fld BI opatch."
				opdir="$fld"
				...
				# Apply opack
				echo "opackcmd.sh apply \"$opdir\""
				(
					if [ "${thisplat#win}" != "$thisplat" ]; then
						opdirw=`echo $opdir | sed -e "s/\//\\\\\/g"`
						opackcmd.sh apply "$opdir" < _tmpimp_
						sts=$?
						echo $sts > _sts_
						unset hhback
					else
					...
					rm -rf _tmpimp_ > /dev/null 2>&1
				) | while read -r line; do print -r "BIPATCH:$line"; done
				sts=`cat _sts_ 2> /dev/null`
				[ -z "$sts" ] && sts=0
				[ "$sts" -ne 0 ] && break	
				readme=
				if [ -f "$fld/README.htm" ]; then
					readme=$fld/README.htm
				elif [ -f "$fld/README.html" ]; then
					readme=$fld/README.html
				fi
				if [ -n "$readme" ]; then
					readme=`cat $readme 2> /dev/null | grep "<title>" | tr -d '\r'`
					readme="${readme#*<title>}"
					readme="${readme%</title>*}"
					[ -n "$readme" ] && readme=" $readme"
					bik=${readme#*\) }
					if [ "$bik" = "${bik#*\(}" ]; then
						bik=${bik#*\(}
						bik=${bik%\)*}
				...
				echo "$bik:$refid # ${readme#*\) }" >> $HYPERION_HOME/opack_ids.txt 2> /dev/null
				echo "${fld}${readme}" >> $HYPERION_HOME/bipatch_version.txt 2> /dev/null
			else
				echo "### $fld is not BI patch folder. Skip it."
			fi
		done
		sts=`cat $HYPERION_HOME/OPatch/_sts_ 2> /dev/null`
		rm -f $HYPERION_HOME/OPatch/_sts_
		[ -z "$sts" ] && sts=0
		exit $sts
	)
...
#	start_service.sh
	[ "$sts" -eq 0 ] \
		...
		|| echo "### biinst.sh:`date +%D_%T` BI PATCH installer retuned $sts."
	exit $sts
fi
