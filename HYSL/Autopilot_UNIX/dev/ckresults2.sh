...
case $AP_SECMODE in
	hss)	echo "RUNNING UNDER HSS MODE.";;
	fa)	echo "RUNNING UNDER FA MODE.";;
	bi|rep)	echo "RUNNING UNDER BI MODE.";;
	*)	echo "RUNNING UNDER NATIVE MODE.";;
esac

echo "======================================================================================="
# Bug 17715224 - ADD OPATCH ID TO THE RTF FILE
if [ -f "$HYPERION_HOME/opack_ids.txt" ]; then
	echo "OPACK Details:"
	(IFS=
		cat $HYPERION_HOME/opack_ids.txt | while read line; do
			echo "  $line"
		done
	)
	echo "======================================================================================="
