...
# 2013/07/23	YKono	Bug 17207430 - SE.SH CREATE WRONG ARBORPATH WITH BI/FA SECURITY MODE.
# 2014/01/22	YKono	Bug 18119705 - SE.SH FAILED TO DEFINE $ODBCINI VARIABLE WITH BISHIPHOME INSTALLATION. 
...
while [ -n "$1" ]; do
	case $1 in
		...
		-nf|-nof|-noforce|noforce|noarbor)
			_sc_force="false"
			;;
		-binat|binat|-bin|bin|-binative|binative)
			[ -z "$_OPTION" ] && export _OPTION="secmode(binat)" || export _OPTION="$_OPTION secmode(binat)"
			;;
		-fa|fa)
			[ -z "$_OPTION" ] && export _OPTION="secmode(fa)" || export _OPTION="$_OPTION secmode(fa)"
			;;
		-birep|birep|-rep|rep|-bi|bi)
			[ -z "$_OPTION" ] && export _OPTION="secmode(rep)" || export _OPTION="$_OPTION secmode(rep)"
			;;
		...
		-o|-opt|opt)
			...
			;;
		-hss|hss)
			[ -z "$_OPTION" ] && export _OPTION="secmode(hss)" || export _OPTION="$_OPTION secmode(hss)"
			;;
		*)
			_sc_ver=$1
			;;
...
for _var in _ESSDIR _ESSCDIR _SXR_CLIENT_ARBORPATH _RTCDIR _PATH _LIB _ARBORPATH _INSTANCE_LOC; do
	_val=`eval echo \\$${_var}`
...
if [ "$AP_SECMODE" = "hss" -o "$AP_SECMODE" = "rep" -o "$AP_SECMODE" = "fa" -o "$AP_SECMODE" = "bi" ]; then
	if [ -z "$ORACLE_INSTANCE" ]; then
		...
		setvar domainhome "\$HYPERION_HOME/../user_projects/domains/bifoundation_domain"
	fi
	export _ARBORPATH=`eval "echo \"$_ARBORPATH\""`
else
	setvar ORACLE_INSTANCE "\$HYPERION_HOME/instance_dummy"
fi
...
if [ "$AP_SECMODE" = "hss" ]; then
	...
	_psmode=":fa"
elif [ "$AP_SECMODE" = "rep" -o "$AP_SECMODE" = "bi" ]; then
	echo "## Set Variables for BISHIPHOME configured environment."
	setvar SXR_ESSBASE_INST 2
	setvar SXR_USER weblogic
	setvar SXR_PASSWORD welcome1
...
if [ "$_sc_client" = "true" ]; then
	...
	if [ -z "$ARBORPATH" ]; then
		...
		for one in $_SXR_CLIENT_ARBORPATH $_ARBORPATH $_ESSCDIR $_ESSDIR $_RTCDIR; do
	...
	if [ -z "$ODBC_HOME" -a -n "$_ODBC_HOME" ];then
		echo "## No \$ODBC_HOME defined (ODBCINST=$ODBCINST)"
		fnd=
		if [ -n "$ODBCINST" ]; then
			fnd=${ODBCINST%/*}
			fnd=${fnd##${HYPERION_HOME}/}
			[ ! -d "$HYPERION_HOME/$fnd" ] && unset fnd
		fi
		if [ -z "$fnd" ]; then
	...
	## Process $_SXR_*_LIB (_SXR_*_LIB first, then _LIBLIST)
	lpath=${_LIBPATH%% *}
	for _libp in SXR_SERVER_LIB SXR_CLIENT_LIB; do
		tmplist=
		libs=`eval echo \\$_${_libp}`
		[ -z "$libs" ] && continue
		echo "## Check \$$_libp"
		for item in $libs; do # reverse list
		...
		[ -n "$_libpath" ] && setvar $_libp "\$${lpath}"
		for item in $tmplist; do
			if [ -n "`echo $item | grep \\$ODBC_HOME`" -a -z "$ODBC_HOME" ]; then
				echo "##   Found $item in the \$_$_libp."
				echo "##     But \$ODBC_HOME not defined."
				echo "##     Skip adding $item to \$$_libp environment variable."
				continue
			fi
			...
			if [ -n "`echo $item | grep \\$ARBORPATH`" -a -z "$ARBORPATH" ]; then
				echo "##   Found $item in the \$_$_libp."
				echo "##     But \$ARBORPATH not defined."
				echo "##     Skip adding $item to \$$_libp environment variable."
				continue
			fi
			if [ -n "`echo $item | grep \\$ESSBASEPATH`" -a -z "$ESSBASEPATH" ]; then
				echo "##   Found $item in the \$_$_libp."
				echo "##     But \$ESSBASEPATH not defined."
				echo "##     Skip adding $item to \$$_libp environment variable."
				continue
			fi
			if [ -n "`echo $item | grep \\$HYPERION_HOME`" -a -z "$HYPERION_HOME" ]; then
				echo "##   Found $item in the \$_$_libp."
				echo "##     But \$HYPERION_HOME not defined."
				echo "##     Skip adding $item to \$$_libp environment variable."
				continue
			fi
			if [ -n "`echo $item | grep \\$SXR_CLIENT_ARBORPATH`" -a -z "$SXR_CLIENT_ARBORPATH" ]; then
				echo "##   Found $item in the \$_$_libp."
				echo "##     But \$SXR_CLIENT_ARBORPATH not defined."
				echo "##     Skip adding $item to \$$_libp environment variable."
				continue
			fi
			if [ "$thisplat" != "${thisplat#win}" ]; then
				_libpath=`eval print -r \\$$_libp | sed -e "s/\\\\\\/\\\//g" -e "s/;/:/g"`
			else
				_libpath=`eval echo \\$$_libp`
			fi
			[ -z "$_libpath" ] \
				&& setvar $_libp "$item" \
				|| setvar $_libp "${item}${pathsep}\$${_libp}"
		done
	done
	unset lpath item tmplist realpath added libs

	## Process $_LIBLIST (_LIB first, then _LIBLIST)
	tmplist=
	for item in $_LIB; do # reverse list
		[ -z "$tmplist" ] && tmplist=$item || tmplist="$item $tmplist" 
	done
	for lpath in $_LIBPATH; do
		[ -z "$tmplist" ] && continue
	...
	for _pnm in SXR_SERVER_PATH SXR_CLIENT_PATH PATH; do
		echo "## Check \$$_pnm"
		val=`eval echo \\$_${_pnm}`
		[ -z "$val" ] && continue
		tmplist=
		for item in $val; do # reverse list
			[ -z "$tmplist" ] && tmplist=$item || tmplist="$item $tmplist" 
		done
		val=`eval echo \\$${_pnm}`
		[ -z "$val" ] && setvar $_pnm "\$PATH"
		for item in $tmplist; do
			if [ -n "`echo $item | grep \\$ODBC_HOME`" -a -z "$ODBC_HOME" ]; then
				echo "##   Found $item in the \$_${_pnm}."
				echo "##     But \$ODBC_HOME not defined."
				echo "##     Skip adding $item to \$${_pnm} environment variable."
				continue
			fi
			if [ -n "`echo $item | grep \\$JAVA_HOME`" -a -z "$JAVA_HOME" ]; then
				echo "##   Found $item in the \$_${_pnm}."
			...
			setvar $_pnm "${item}${pathsep}\$$_pnm"
		done
	done # _PATH, _SXR_CLIENT_PATH, _SXR_SERVER_PATH
	if [ -z "$SXR_CLIENT_ARBORPATH" ]; then
		echo "## Missing \$SXR_CLIENT_ARBORPATH definition."
		echo "##   Remove SXR_CLIENT_PATH and SXR_CLIENT_LIB definitions."
		[ -n "$SXR_CLIENT_PATH" ] && setvar SXR_CLIENT_PATH ""
		[ -n "$SXR_CLIENT_LIB" ] && setvar SXR_CLIENT_LIB ""
	fi
	unset item tmplist realpath _pnm val
fi
...
# # Adjust SXR_DBHOST
# if [ "$AP_SECMODE" = "hss" -o "$AP_SECMODE" = "rep" -o "$AP_SECMODE" = "bi" ]; then
...
if [ -n "$_sc_env" ]; then
	...
	echo ". fixpath.sh -v SXR_SERVER_PATH SXR_SERVER_LIB SXR_CLIENT_PATH SXR_CLIENT_LIBG	# Added by autopilot" >> ${_sc_env}
fi
...
. fixpath.sh -v SXR_SERVER_PATH SXR_SERVER_LIB SXR_CLIENT_PATH SXR_CLIENT_LIB
...
unset _ARBORPATH _extpath _EXTRA_SETENV _fnd
unset _IGNORE_OPACK _INSTANCE_LOC _iswin _libp
unset _one _path _pathsep _pathtmp _RTCDIR
unset _skipesslang _SXR_CLIENT_PATH _SXR_SERVER_PATH
unset _v _var _varlist _verbose
