...
# 05/23/2012 YKono Remove check task part to ckmytsk.sh
# 07/09/2012 YKono Reset _OPTION string before run start_regress.sh

. apinc.sh

aplckf="$AUTOPILOT/lck/${LOGNAME}@$(hostname).ap"
trap 'unlock.sh "$aplckf" > /dev/null 2>&1 ; echo "`date +%D_%T`:autopilot.sh exit" > ~/ap.log ' EXIT
...
chk_tp()
{
	...
	if [ ! -f "$amsal" ]; then
		echo "`date +%D_%T`:No apmon.sh. Launch it."
		echo $$ > "$ampar"
...
cmdf="$AUTOPILOT/mon/${thisnode}.cmd"
if [ -f "$cmdf" ]; then
	echo "`date +%D_%T`:### Found command file ($cmdf)."
	lcnt=0
	lock.sh "$cmdf" -m "Read cmd file($thisnode:$thisplat)." $$ > /dev/null 2>&1
	sts=$?
	while [ "$sts" -ne 0 -a "$lcnt" -lt 30 ]; do
		if [ -f "${cmdf}.lck" ]; then
			echo "`date +%D_%T`:### Command file is locked by:"
		...
		sleep 1
		let lcnt=lcnt+1
		lock.sh "$cmdf" -m "Read cmd file - retry($thisnode:$thisplat)." $$ > /dev/null 2>&1
		sts=$?
	done
	if [ "$sts" -eq 0 ]; then
		echo "`date +%D_%T`:### Execute $cmdf."
		chmod +x "$cmdf"
		...
		echo "`date +%D_%T`:### Failed to lock the command file."
		echo "`date +%D_%T`:### Give up to exec $cmdf."
	fi
fi
...
START) #####################################################
[ "$flg" != "$pflg" ] && echo "`date +%D_%T` ${thisnode} $$ $_AP_IN_BG : START." >> $aphis
tsk=`ckmytsk.sh`
sts=$?
if [ -n "$tsk" ]; then
	ret=false
	content=`head -1 "$AUTOPILOT/que/crr/$tsk"`
	parsedby=`head -2 "$AUTOPILOT/que/crr/$tsk" | tail -1`
	orgname=`head -3 "$AUTOPILOT/que/crr/$tsk" | tail -1`
	set_sts RUNNING
	pri=${orgname%%~*}
	ver=`echo $orgname | awk -F~ '{print $2}' | sed -e "s/!/\//g"`
	bld=`echo $orgname | awk -F~ '{print $3}'`
	...
	obld=${obld%%:*}
	# Make ap sal
	write_apsal "$tsk" "PREPARATION" "${tst}" "${ver}:${bld}"
	# Run regression
	export CRRPRIORITY=$pri
	echo "`date +%D_%T`:## RUN <$content> : tsk($tsk)"
	export set_vardef=$AUTOPILOT/mon/${thisnode}.reg.vdef.txt
	rm -rf "$set_vardef" > /dev/null 2>&1
	if [ "$_AP_BG" = "true" ]; then
		if [ "$AP_LOGTO" = "false" ]; then
			start_regress.sh $ver $obld $tst > /dev/null 2>&1
			ret=$?
		else
			rm -rf "$AP_LOGTO" 2> /dev/null
			( start_regress.sh $ver $obld $tst
		  	echo "logto.sh return status=$?"
			) 2>&1 | logto.sh "$AP_LOGTO"
			ret=$?
		fi
	else
		start_regress.sh $ver $obld $tst
	...
	rm -rf "$AUTOPILOT/mon/${thisnode}.reg.vdef.txt" > /dev/null 2>&1
	echo "`date +%D_%T`:Regression results code = $ret."
	...
	if [ $ret -lt 10 ]; then
		...
		if [ "$_suc" -eq 0 -a "$_dif" -eq 0 ]; then
			# Move back task file.
			echo "`date +%D_%T`:# Failed test by 0/0 result."
			[ "${tsk%${thisnode}.tsk}" != "$tsk" ] \
				&& ltrg="${thisnode}.read" || ltrg="${thisplat}.read"
			wcnt=0
			_pid=`lock.sh $AUTOPILOT/lck/${ltrg} -m "Return task by Zero ($thisnode:$thisplat)" $$`
			lsts=$?
			while [ $lsts -ne 0 -a $wcnt -lt 60 ]; do
				sleep 5
				let wcnt=wcnt+1
				_pid=`lock.sh $AUTOPILOT/lck/${ltrg} \
					-m "Return task by Zero-retry ($thisnode:$thisplat)" $$`
				lsts=$?
			done
			if [ $lsts -ne 0 ]; then
				ttsk="$AUTOPILOT/que/${orgname}.zero"
				echo "`date +%D_%T`:# Failed to lock ${ltrg} file."
				echo "`date +%D_%T`:# Couldn't move back $tsk file into the queue."
				echo "`date +%D_%T`:# Move current task to ${tsk}.zero."
				rm -rf "$ttsk" 2> /dev/null
 				echo "$content" > "$ttsk"
 				echo "$parsedby" >> "$ttsk"
 				echo "Ret:$ret(${thisnode} `date +%D_%T`)" >> "$ttsk"
			else
				ttsk="$AUTOPILOT/que/${orgname}"
				echo "`date +%D_%T`:# Move back current task to the queue."
				rm -rf "$ttsk" 2> /dev/null
 				echo "$content" > "$ttsk"
 				echo "$parsedby" >> "$ttsk"
 				echo "Ret:$ret(${thisnode} `date +%D_%T`)" >> "$ttsk"
				unlock.sh $AUTOPILOT/lck/${ltrg} > /dev/null 2>&1
			fi
		else
			ttsk="$AUTOPILOT/que/done/${tsk}"
			mv "$AUTOPILOT/que/crr/${tsk}" "$ttsk"
			echo "End:`date +%D_%T`" >> "$ttsk"
			echo "Suc:$_suc, Dif:$_dif" >> "$ttsk"
			echo "Ret:$ret(${thisnode} `date +%D_%T`)" >> "$ttsk"
			echo "`date +%D_%T`:## Wait for the next task..."
		fi
	elif [ $ret -lt 20 ]; then
		[ "${tsk%${thisnode}.tsk}" != "$tsk" ] \
			&& ltrg="${thisnode}.read" || ltrg="${thisplat}.read"
		wcnt=0
		_pid=`lock.sh $AUTOPILOT/lck/${ltrg} -m "Return task by < 20 ($thisnode:$thisplat)" $$`
		lsts=$?
		while [ $lsts -ne 0 -a $wcnt -lt 60 ]; do
			sleep 5
			let wcnt=wcnt+1
			_pid=`lock.sh $AUTOPILOT/lck/${ltrg} \
				-m "Return task by < 20 - retry ($thisnode:$thisplat)" $$`
			lsts=$?
		done
		if [ $lsts -ne 0 ]; then
			ttsk="$AUTOPILOT/que/${orgname}.move"
			echo "`date +%D_%T`:# Failed to lock ${ltrg} file."
			echo "`date +%D_%T`:# Couldn't move back $tsk file into the queue."
			echo "`date +%D_%T`:# Move current task to ${tsk}.move."
			rm -rf "$ttsk" 2> /dev/null
 			echo "$content" > "$ttsk"
 			echo "$parsedby" >> "$ttsk"
 			echo "Ret:$ret(${thisnode} `date +%D_%T`)" >> "$ttsk"
		...
		ttsk="$AUTOPILOT/que/${orgname}.skipped"
		echo "$content" > "$ttsk"
		echo "$parsedby" >> "$ttsk"
		echo "Skipped on ${thisnode} at `date +%D_%T`" >> "$ttsk"
		echo "Return code = $ret" >> "$ttsk"
	fi
	# Maintain AP_RTFILE
	if [ "$ret" != "0" ]; then
		flg=`get_flag`
		...
		if [ "${flg}" != "$tmp" ]; then
			...
			if [ "$tmp" != "$flg" ]; then
				set_flag "KILL/$flg"
				ttsk="$AUTOPILOT/que/${orgname}.skipped"
				mv  "$AUTOPILOT/que/crr/${tsk}" "$ttsk" 2> /dev/null
				echo "Skipped on ${thisnode} at `date '+%D_%T'`" >> "$ttsk"
			fi
		fi
	else
	...
	rm -f "$AUTOPILOT/que/crr/$tsk" > /dev/null 2>&1
	set_sts WAITING
	sleep 5
else
	sleep 30
...
STOP) #############################################################################
echo "`date +%D_%T`:Receive Stop command."
set_sts "HALT"
rm -f "$amsal"
sleep 5
...
PAUSE) #############################################################################
echo "`date +%D_%T`:Receive Pause command."
echo "`date +%D_%T`:Waiting another command in the flag file."
[ "`get_sts`" != "KILLED" ] && write_apsal "PAUSE" both
set_sts "IDLE"
...
write_apsal "IDLE" both
set_sts "WAITING"
echo "`date +%D_%T` $thisnode $$ $_AP_IN_BG : PAUSE." >> $aphis
;;

KILL*) #############################################################################
flg=${flg#KILL/}
echo "`date +%D_%T`:Receive KILL command."
echo "`date +%D_%T`:Killed current regression and move to $flg mode."
set_flag "$flg"
echo "`date +%D_%T` $thisnode $$ $_AP_IN_BG : KILL($flg)." >> $aphis
;;
...
RESTART) ###########################################################################
echo "`date +%D_%T`:Receive Restart command"
echo "`date +%D_%T`:Clear log(previous history) and start again."
rm -f $AP_TSKDONE/*~${node}.tsk 2> /dev/null
set_flag "START"
echo "`date +%D_%T` $thisnode $$ $_AP_IN_BG : RESTART." >> $aphis
;;

*) #################################################################################
echo "`date +%D_%T`:Unkown FLAG($flg)"
...
if [ -f "$apsal" ]; then
	...
	if [ -z "$tmp" ]; then
		echo "`date +%D_%T`:There is an apsal. But the process($appid) is not found."
		echo "`date +%D_%T`:Delete $apsal."
		unset appid
		rm -f "$apsal"
	fi
fi
...
if [ $? -ne 0 ]; then
	echo "`date +%D_%T`:Failed to lock \"$aplckf\"(sts=$?)."
	if [ -f "$aplckf" ]; then
		cat "$aplckf" 2> /dev/null | while read line; do
			echo "> $line"
		done
	fi
	echo "`date +%D_%T`:Please check there is another autopilot.sh running ?"
	echo "`date +%D_%T`:Or you can access to the \$AUTOPILOT/lck folder ?"
	exit 1
fi
...
case "$action" in
	...
	start)
		...
		echo "`date +%D_%T`:Parameter is start"
		echo "`date +%D_%T` $thisnode $$ $_AP_IN_BG : LAUNCH." >> $aphis

		[ -n "$appid" ] && echo "`date +%D_%T`:There is another autopilot process.($appid)"
		set_flag START
		[ -z "$appid" ] && run_autopilot
	...
	stop)
		...
		echo "`date +%D_%T`:Parameter is stop"	

		if [ -n "$appid" ]; then
			echo "`date +%D_%T`:There is another autopilot process.($appid)"
			echo "`date +%D_%T`:Stop that process."
			set_flag STOP
			echo "`date +%D_%T`:Waiting for another autopilot done."
			while [ "`get_sts`" != "HALT" ]; do
				sleep 5
			done
		fi
	...
	pause)
		...
		echo "`date +%D_%T`:Parameter is pause"

		if [ -n "$appid" ]; then
			echo "`date +%D_%T`:There is another autopilot process.($appid)"
			echo "`date +%D_%T`:Set the flag file to pause for that process."
			set_flag PAUSE
		else
			set_flag PAUSE
			run_autopilot
	...
	restart)
		...
		echo "`date +%D_%T`:Parameter is restart"

		if [ -n "$appid" ]; then
			echo "`date +%D_%T`:There is another autopilot process.($appid)"
			echo "`date +%D_%T`:Stop that process and will re-start autopilot task on this shell."
			set_flag STOP
			echo "`date +%D_%T`:Waiting for another autopilot done."
			while [ "`get_sts`" != "HALT" ]; do
				sleep 5
	...
	reset)
		...
		echo "`date +%D_%T`:Parameter is reset"

		if [ -n "$appid" ]; then
			echo "`date +%D_%T`:There is another process.($appid)"
			echo "`date +%D_%T`:Set the flag for that process to RESTART."
			set_flag RESTART
		else
			echo "`date +%D_%T`:There is no autopilot process."
		fi
	...
	KILL/*)
		...
		echo "`date +%D_%T`:Parameter is $action"

		if [ -n "$appid" ]; then
			echo "`date +%D_%T`:There is another process.($appid)"
			echo "`date +%D_%T`:Set the flag for that process to KILL."
			set_flag "${action}"
		else
			echo "`date +%D_%T`:There is no autopilot process."
		fi
	...
	help)
		mysh=`which $0`
		lno=`cat $mysh 2> /dev/null | grep -n "^#HELPCONT"`
		lno=${lno%%:*}
	...
	*)
		...
		if [ -n "$action" ]; then
			echo "`date +%D_%T`:Illegal action = $action"
			echo "`date +%D_%T`:autopilot.sh {start|restart|stop|pause|KILL*}"
			echo "`date +%D_%T`:   [-noinst|-force|-refresh|-cd|-both|-noagtpsetup|-noenv|"
			echo "`date +%D_%T`:    -waittime=#|-keepwork=#|-env <env>|-tsk <tsk>]"
		fi

		if [ -n "$appid" ]; then
			isidle=`tail -1 "$apsal" 2> /dev/null`
			if [ "$isidle" = "IDLE" ]; then
				echo "`date +%D_%T`:autopilot.sh($appid) is IDLE."
			else
				if [ -f "$AP_RTFILE" ]; then
					cont=`cat "$AP_RTFILE"`
			...
			echo "`date +%D_%T`:There is no autopilot process on $(hostname):${LOGNAME}"
		fi
...
AP_LOGTO : The log file name for the regression output.
Refered from:
    autopilot.sh
Values and meaning:
    If this value is defined, the autopilot.sh log to $AP_LOGTO file.
    This option is made for running autopilot from none-Utah location.
    The log output of regression takes a lot of transfer time during
    test. Then set this value to local storage, it will be shorten.
    But when you define this to local place, you cannot use see the log
    file from other machine via aplog.sh.
    If you define this variable to "false", autopilot never create a
    log file for the regression.

AP_PLATGROUP : The platform group for ignplat() or runplat() task option.
Refered from:
    ckmytsk.sh
Values and meaning:
    If you want to group some machines for specific execution, you can define
    this value to specific value and can use that value in ignplat() or 
    runplat() option.
    i.e.)  When you want to run test only HP-UX 11.23 with following machines:
        hpux64 - regrrk1@stiahp3 (11.23) - export AP_PLATGROUP=HP1123
        hpux64 - regrrk2@stiahp3 (11.23) - export AP_PLATGROUP=HP1123
        hpux64 - regrrk1@stiahp4 (11.31)
        hpux64 - regrrk2@stiahp4 (11.31)
        You can define AP_PLATGROUP to HP1123 and use following task option
        to run test.
        $ addtsk.sh 11.1.2.2.100 2166 agtpori1.sh -o "runplat(HP1123)"
        $ addtsk.sh 11.1.2.2.100 2166 agtpori2.sh -o "runplat(HP1123)"
        Those tasks are not executed by stiahp4 because they don't have
        AP_PLATGROUP=HP1123 definition.
...
FreeMem(<size>) : The free memory amount to run the test.
        If current free memory is lower than <size>, autopilot move to PAUSE mode
        and send e-mail notification.
        <size> := <number>[KB|MB|GB]
GTLFRelease(<gtlf-release>) : GTLF Releases string.
GTLFOS(<ap-plat>) : Define the different OS name for GTLF OS.
        <ap-plat> := The platform name in the autopilot framework.
                     This name automatically convert to GTLF platform name.
GTLFLoad(<gtlf-load>) : GTLF Load string.
GTLFTestUnit(<gtlf-release>) : GTLF Test unit string.
HSS(true) : Run test with HSS mode.
