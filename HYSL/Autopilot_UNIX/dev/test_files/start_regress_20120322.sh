...
if [ "$noinst" != "true" ]; then
	...
	if [ "$AP_INSTALL" != "false" ]; then
		...
		if [ "$_force" = "true" ]; then
			...
			_inst_err=$?
			echo "$thisnode($thisplat):`get_instkind.sh -l`" >> \
...
# echo "$difname	$difbld	$difnum[$difdesc]	$shlist"
	...
	gtlfunitf="GTLF_testunit.txt"
	tstscr=${TEST%% *}
	fpth=
	rm -f $VIEW_PATH/autoregress/work/${gtlfunitf} 2> /dev/null
	[ -f "$VIEW_PATH/autoregress/sh/$tstscr" ] && fpth="$VIEW_PATH/autoregress/sh/$tstscr"
	[ -z $fpth -a -f "$SXR_HOME/../base/sh/$tstscr" ] && fpth="$SXR_HOME/../base/sh/$tstscr"
	if [ -n "$fpth" ]; then
		tstunit=`grep "^#:[     ][      ]*GTLF_testunit:" "${fpth}" | tr -d '\r'`
		tstunit=${tstunit#*GTLF_testunit:}
		tstunit=`echo $tstunit | awk '{print $1}'`
		echo "$tstunit" > $VIEW_PATH/autoregress/work/${gtlfunitf}
	fi
