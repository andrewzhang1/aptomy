...
while [ $# -ne 0 ]; do
	case $1 in
		...
		-f)
			...
			;;
		win32|linuxamd64|linux|winamd64|hpux64|solaris64|solaris|aix64|aix|all)
			plt=$1
			;;
		-p)
			shift
			if [ $# -eq 0 ]; then
				echo "There is no platform string for -p option."
				display_help
				exit 1;
			else
				plt="$1"
...
if [ -d "$src" ]; then
	...
	if [ "$skipsizecheck" = "false" -a ! -d "${src}/HIT${pltx}" ]; then
		echo "No \"${src}/HIT${pltx}\" folder exist."
		exit 3
	fi
else
...
if [ "$sizecheck" = "true" ]; then
	...
	srcsize=`du -k -t "${src}/HIT${pltx}" | tail -1 | awk '{print $1}'`
	trgsize=`get_free.sh "$trg"`
	echo "Source HIT${pltx} size  = $srcsize"
...
if [ -n "$plt" ]; then
	...
	ignplt=
	for one in $allplat; do
		if [ "$one" != "$plt" ]; then
			[ -z "$ignplt" ] && ignplt=$one || ignplt="$ignplt|$one"
		fi
	done
	echo "IgnPlatform=$ignplt"
	ls *.zip | egrep "$fltr" | egrep -v "$ignplt" | while read onezip; do
		cp "$onezip" "$trg"
		cd "$trg"
		${unzipcmd} -o "$onezip"
		rm -f "$onezip"
