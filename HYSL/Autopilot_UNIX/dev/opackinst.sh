...
# Function: apply_one_opack <opdir> <kind> # kind := server/client/rtc/aps/eas/studio
apply_one_opack()
{
	_pwd=`pwd`
	...
	[ -z "$lsts" ] && lsts=0
	cd $_pwd
	echo "$2:$refid	# $3" >> $HYPERION_HOME/opack_ids.txt 2> /dev/null
	return $lsts
}
...
apply_opack()
{
	for opdir in $opdirs; do
		...
		if [ "$skipf" = "false" ]; then
			...
			apply_one_opack $opdir $whichOpack $zipname
			sts=$?
			...
			if [ $sts -eq 0 ]; then
				echo "### Applied ${opdir}:${whichOpack}(sts=0)."
			else
				echo "### Failed to apply ${opdir}:${whichOpack}(sts=$sts)."
				let sts=sts+10
				break
			fi
		else
...
while [ 1 ]; do
	...
	rm -rf unzip.log > /dev/null 2>&1
	[ -z "$opdirs" ] && opdirs="$opdir:$whichOpack:$tarzip" || opdirs="$opdirs $opdir:$whichOpack:$tarzip"
...
if [ "$sts" -eq 0 ]; then
	...
	for one in $target; do
		...
		if [ "$tarbld" = "latest" ]; then
			tarbld=`get_latest.sh $one $ver`
			echo "# $one:$ver:latest=$tarbld($?)"
		fi
		badbld=
		while [ 1 ]; do
			if [ -d "$rootf/$ver/$tarbld/opack" ]; then
				if [ "$one" = "eas" ]; then
					pl=`get_platform.sh`
					ls $rootf/$ver/$tarbld/opack/$pl/server/*.zip > /dev/null 2>&1
				else
					ls $rootf/$ver/$tarbld/opack/*.zip > /dev/null 2>&1
				fi
				[ $? -eq 0 ] && break
				echo "# - No zip files for $tarbld"
			else
				echo "# - No opack folder for $one $ver:$bld."
			fi
			[ -z "$badbld" ] && badbld=$tarbld || badbld="$badbld $tarbld"
			tarbld=`get_latest.sh $one $ver $badbld`
			echo "# $one:$ver:latest=$tarbld($?)"
			[ -z "$tarbld" ] && break
		done
	...
	echo "### APPLYOTHER $one $tarzip(${kind}:$finddir)"
					apply_one_opack $finddir $one "$ver:$tarbld"
					echo "# Done $one $tarzip($kind) opack. status=$?"
					echo "$ver:$tarbld" > $HYPERION_HOME/${one}_version.txt
					[ -z "$zips" ] && zips=$tarzip || zips="$zips $tarzip"
				fi
			done
			if [ -z "$zips" ]; then
				echo "# There is no valid $one zip file under `pwd`"
			fi
			cd $crr
		else
			echo "# There is no valid $one opack build for $ver."
		fi
	done
fi
