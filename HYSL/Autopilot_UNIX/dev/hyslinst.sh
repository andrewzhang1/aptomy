...
# echo "### _OPTION=$_OPTION"
	...
	if [ "$sts" -eq 0 ]; then
		(	. se.sh $ver > /dev/null 2>&1
			...
			if [ "$sts" -eq 0 -a "$thisplat" != "${thisplat#win}" ]; then
				rm -f $HYPERION_HOME/missingmsi.txt
				ops=`chk_para.sh fakemsi "$_OPTION"`
				ops=${ops##* }
				[ "$ops" = "true" ] && ops=true || unset ops
				if [ -f "$BUILD_ROOT/$ver/$ebld/client/EssbaseClient.exe" ]; then
					...
					echo "### ERROR : Client MSI Installer not found. ###"
					echo "Missing Client MSI Installer." >> $HYPERION_HOME/workaround.txt
					echo "Missing Client MSI Installer." >> $HYPERION_HOME/missingmsi.txt
					rtcloc=${PATH##*EssbaseRTC}
					rtcloc="$HYPERION_HOME/common/EssbaseRTC${rtcloc%%;*}"
					if [ -d "$rtcloc" ]; then
						if [ -d "$SXR_CLIENT_ARBORPATH" ]; then
							echo "### Copy RTC binaries to ${SXR_CLIENT_ARBORPATH##*/}/bin folder."
							echo "# Copy RTC binaries to EssbaseClient/bin folder." >> $HYPERION_HOME/workaround.txt
							cp $rtcloc/* $SXR_CLIENT_ARBORPATH/bin 2> /dev/null
						else
							echo "### Found EssbaseRTC folder($rtcloc)."
							echo "### But no \$SXR_CLIENT_ARBORPATH definition. Skip copying RTC binaries."
						fi
					else
						echo "### But couldn't find EssabseRTC folder."
						echo "### Just skip copying RTC binaries."
					fi
					if [ -d "$SXR_CLIENT_ARBOTPATH/bin" ]; then
						echo "### Copy client program to \$SXR_CLIENT_ARBORPATH/bin."
						cp -f $ESSBASEPATH/bin/ESSCMD.exe $SXR_CLIENT_ARBORPATH/bin 2> /dev/null
						cp -f $ESSBASEPATH/bin/essmsh.exe $SXR_CLIENT_ARBORPATH/bin 2> /dev/null
					fi
				else
					echo "### ERROR : Client MSI Installer not found. ###"
					sts=127
...
(
	. se.sh $ver > /dev/null 2>&1
	if [ -d "$SXR_CLIENT_ARBORPATH" ]; then
		mv $SXR_CLIENT_ARBORPATH $SXR_CLIENT_ARBORPATH.refresh
	fi
)
