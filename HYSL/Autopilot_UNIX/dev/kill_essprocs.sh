...
# 2014/02/07 YKono	Add AP_KILLESSPROCS hook for AIX64 workaround
#                       When define AP_KILLESSPROCS to the valid script, this script
#			call target script. In the target script, you need kill the
#			processes which lock the files.
...
if [ -n "$AP_KILLESSPROCS" -a -x "$AP_KILLESSPROCS" ]; then
	$AP_KILLESSPROCS
fi
exit 0
