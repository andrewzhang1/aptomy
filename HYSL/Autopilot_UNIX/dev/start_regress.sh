...
# 2014/01/21 YKono Bug 18078885 - FIND A WAY TO AUTOMATE THE PROCESS TO COMPARE THE DIF FROM DIFFERENT BUILD
#######################################################################
...
if [ ! -d "${VIEW_PATH}/autoregress" ]; then
	...
	if [ "$_tmp_" != "false" ]; then
		for fld in work bin csc data log msh rep scr sh sxr; do
			...
			rm -rf ${VIEW_PATH}/autoregress/${fld}/* 2> /dev/null
			# ls -a ${VIEW_PATH}/autoregress/${fld} | while read one; do
			# 	if [ "$one" != "." -a "$one" != ".." ]; then
			# 		# echo "###   - remove ${fld}/${one}"
			# 		rm -rf ${VIEW_PATH}/autorregress/${fld}/${one}
			# 	fi
			# done
		done
	else
		echo "###   Skip clean-up view folders by cleanupViewFolders($_tmp_) option."
	fi
fi
...
if [ "$verbose" = "true" ]; then
	...
	# renameform="${AP_DEF_RESPATH}/${_platname}_${VERSION##*/}_${BUILD}${TAG}_${TEST_ABV}.ana"
	searchform="${_platname}_${VERSION##*/}_${BUILD}${TAG}_${TEST_ABV}*.ana"
	cd ${AP_DEF_RESPATH}
	...
	renameform="${AP_DEF_RESPATH}/${_platname}_${VERSION##*/}_${BUILD}${TAG}_${TEST_ABV}_${lastfile}.ana"
	...
	if [ -f "$AUTOPILOT/mon/${mynode}.mon.kill.rec" ]; then
		...
		echo "*************************************************************************"	>> $anafile
		cat $AUTOPILOT/mon/${mynode}.mon.kill.rec | while read one; do
			echo "  $one" >> $anafile
		done
		echo "" >> $anafile
		rm -f $AUTOPILOT/mon/${mynode}.mon.kill.rec
	fi
	cp $anafile anafile
	echo "Suc $succnt.	Dif $difcnt." >> $anafile
	echo 	>> $anafile
	[ -f "difs.rec" ] && rm -rf difs.rec > /dev/null 2>&1
	[ -f "ndifs.rec" ] && rm -rf ndifs.rec > /dev/null 2>&1
	...
	if [ $difcnt -ne 0 ]; then
		...
		if [ -f "${TEST%.*}.sta" ]; then
			analyze.pl ${TEST%.*} $AUTOPILOT/data/dif_script.txt $AUTOPILOT/data/ignoresh.txt | tr -d '\r' > difs.txt
			if [ $? -ne 0 ]; then
				echo "#ANALYZE ERROR" > difs.txt
				ls *.dif > difs.txt
			fi
			mk_difrec.sh ${VERSION##*/} ${BUILD} ${_platname} ${TEST_ABV} ctdifs.txt < difs.txt
			keepnth.sh "${AUTOPILOT}/dif/${VERSION##*/}~\*~${_platname}~${TEST_ABV}.rec" 10
			...
			if [ -f "ctdifs.txt" ]; then
				cat ctdifs.txt | while read shlist; do
					...
					# 	ctdif=
					# fi
					ctdif=" $difdesc(first build $difbld:$difnum consecutive diffs)"
					[ "$difnum" -eq 1 ] && ctdif="$ctdif (New diff)"
					unset _ignorable
					######################################
					...
					[ "$difnum" -eq 1 ] && echo "${difname} ${_sc_ow}(${shlist})${ctdif}${_ignorable}" >> ndifs.rec
				done	
	...
	########################################################################
	# send New dif notification to each owner
	TAG=`chk_para.sh tag "$_OPTION"`
	ndfplt=`get_platform.sh -l`
[ -f "ndif.rec" ] && echo "### Found ndif.rec." || echo "### There is no NDIF.REC"
	[ -f "ndif.rec" ] && send_email.sh $AUTOPILOT/form/32_NewDif.txt "ne:ndif.rec"
	if [ "$AP_NOPLAT" = "false" ]; then
		ttlscr=`cat $AUTOPILOT/tsk/${ndfplt}.tsk 2> /dev/null | grep "^${VERSION##*/}" | grep -v "i18n" | grep -v "xprobmain" | wc -l`
		nmext=${ndfplt}
	else
		ttlscr=`cat $AUTOPILOT/tsk/${LOGNAME}.tsk 2> /dev/null | grep ":${VERSION##*/}:" | grep -v "i18n" | grep -v "xprobmain" | wc -l`
		nmext=${LOGNAME}
	fi
	let ttlscr=ttlscr 2> /dev/null
	if [ $ttlscr -ne 0 ]; then
		wrtstg "#PROCESS NEW DIFF RECORD"
		ndffnm="$AUTOPILOT/mon/newdif_${VERSION##*/}_${BUILD}${TAG}_${nmext}.rec"
		ndc=`cat ndif.rec 2> /dev/null | wc -l`
		let ndc=ndc 2> /dev/null
		echo "Test:$TEST=$ndc" >> $ndffnm
echo "### New diff:cnt=$ndc"
		[ -f "ndif.rec" ] && cat ndif.rec >> $ndffnm 2> /dev/null
		ndc=`cat $ndffnm 2> /dev/null | grep "^Test:" | while read line; do
			echo ${line%%=*}
		done | sort | uniq | wc -l`
		let ndc=ndc 2> /dev/null
echo "### New diff:tstcnt=$ndc(ttl=$ttlscr)"
		if [ "$ndc" -ge "$ttlscr" ]; then
			ttl=`cat $ndffnm 2> /dev/null | wc -l`
			let ttl=ttl-ndc 2> /dev/null
echo "### New duff:difcnt=$ttl."
			if [ "$ttl" -gt 0 ]; then
				# SEND NEW DIFF NOTIFICATION
echo "### New duff:Send notification."
				send_email.sh $AUTOPILOT/form/33_NewDifPlat.txt "ne:$ndffnm"
			fi
			rm -f $ndffnm
		fi
	fi
