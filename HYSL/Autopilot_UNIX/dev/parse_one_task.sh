...
# 09/19/2013 YKono - Bug 17465405 - AUTOPILOT NEEDS TO PICK BUILDS WITH THE SAME BUILD NUMBER
...
wrtwsp2()
{
	resloc=`get_resloc.sh $ver - $tst -o "$opt"`
...
nobldchk=`chk_para.sh NoBuildCheck "$opt"`
[ -z "$nobldchk" ] && nobldchk=`chk_para.sh NoBuildTest "$opt"`
...
for one in aps studio eas; do
	str=`chk_para.sh ${one}Bld "$opt"`; str=${str##* }
	if [ -n "$str" -a "$str" = "latest" ]; then
		str=`get_latest.sh $ver $one`
		[ $? -eq 0 ] && opt="$opt ${one}bld(${str##* })"
	fi
done
# setEnv(APS_BUILD=)
str=`chk_para.sh setenv "$opt"`
str=`echo $str | grep "APS_BUILD=latest"`
if [ -n "$str" ]; then
	str=`get_latest.sh $ver aps`
	[ $? -eq 0 ] && opt="$opt setEnv(APS_BUILD=${str##* })"
fi
...
if [ "$igndone" != "true" -a -f "$AP_TSKDONE/${donename}" ]; then
	nm=`grep "^On:" "$AP_TSKDONE/${donename}" | sed -e "s/On://g" | tr -d '\r'`
	[ -z "$nm" ] && nm="another platform"
