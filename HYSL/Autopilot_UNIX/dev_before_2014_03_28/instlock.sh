...
while [ "${hoststs#succeeded}" = "$hoststs" -o "${usersts#succeeded}" = "$usersts" ]; do
	...
	if [ "${usersts#succeeded}" = "$usersts" -o "${hoststs#succeeded}" = "$hoststs" ]; then
		...
		if [ "${usersts#succeedded}" = "$usersts" -a -f "${userlck}.lck" ]; then
			crrlck=`cat ${userlck}.lck 2> /dev/null`
			if [ "$prevusersts" != "$crrlck" ]; then
				locknd=`cat ${userlck}.lck 2> /dev/null | head -1 | tr -d '\r'`
				locknd=${locknd%% *}
				echo "`date +%D_%T`:${userlck##*/}.lck is locked by $locknd."
				prevusersts=$crrlck
			else
				prevusersts=
			fi
		fi
		if [ "${hostests#succeedded}" = "$hoststs" -a -f "${hostlck}.lck" ]; then
			crrlck=`cat ${hostlck}.lck 2> /dev/null`
			if [ "$prevhoststs" != "$crrlck" ]; then
				locknd=`cat ${hostlck}.lck 2> /dev/null | head -1 | tr -d '\r'`
				locknd=${locknd%% *}
				echo "`date +%D_%T`:${hostlck##*/}.lck is locked by $locknd."
				prevhoststs=$crrlck
			else
				prevhoststs=
			fi
