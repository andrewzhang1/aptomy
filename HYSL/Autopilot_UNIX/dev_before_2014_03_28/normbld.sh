...
while [ $# -ne 0 ]; do
	case $1 in
		...
		-o)
			shift
			if [ $# -eq 0 ]; then
				echo "'=o' need second parameter as option."
				exit 1
			fi
...
s=`chk_para.sh ignCM "$_OPTION"`
s=${s##* }
[ -n "$s" ] && ignCM=$s
if [ -f "$AUTOPILOT/data/ignCM.txt" ]; then
	icmtxt=`cat $AUTOPILOT/data/ignCM.txt 2> /dev/null | grep -v "^#" | grep -v "^$" | grep "^${ver}"`
else
	icmtxt=
fi
[ "$ver" = "talleyrand_ps1" ] && ver=talleyrand_sp1
if [ "$ignCM" != "true" -a -z "$icmtxt" ]; then
	...
	if [ "$ret" != "0" ]; then
		echo "#BLD"
		echo "CM($ignCM) Acceptance was failed for $ver/$bld/$plat."
		exit 5
	fi
fi
