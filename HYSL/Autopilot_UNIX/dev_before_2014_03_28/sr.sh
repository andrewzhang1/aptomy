...
echo "#DEBUG difname=$difname, _owner=$_owner" | tee -a ~/debug.txt
...
cat $msgfile | while read line ; do echo "#DEBUG  $line"; done | tee -a ~/debug.txt
								fi
							else
								msgfile="${msgfile}.msg"
								if [ ! -f "$msgfile" ]; then
...
echo "#DEBUG msgfile=$msgfile" | tee -a ~/debug.txt
...
echo "#DEBUG - ls \$AUTOPILOT/log" | tee -a ~/debug.txt
ls -l $AUTOPILOT/log | while read line; do echo "#DEBUG > $line"; done | tee -a ~/debug.txt
	fi # if [ $difcnt != 0 ]
...
echo "#DEBUG - SEND DIFF : difcnt=$difcnt, _putpdif=$_putpdif" | tee -a ~/debug.txt
echo "#DEBUG - SEND DIFF : mysvnode=$mysvnode, VERSION=${VERSION##*/}" | tee -a ~/debug.txt
echo "#DEBUG - SEND DIFF : BUILD=$BUILD, TEST_ABV=${TEST_ABV}" | tee -a ~/debug.txt
	if [ $difcnt -ne 0 -a "$_putpdif" = "true" ]; then
		echo "Sending e-mail to each script owner"
		cd $AP_DEF_LOGPATH
echo "#DEBUG - SEND DIFF - AP_DEF_LOGPATH=$AP_DEF_LOGPATH, `pwd`" | tee -a ~/debug.txt
ls -l | while read line; do echo "#DEBUG - SEND DIFF > $line" ; done | tee -a ~/debug.txt
...
echo "#DEBUG - SEND DIFF - fname=$fname, target_owner=$target_owner" | tee -a ~/debug.txt
