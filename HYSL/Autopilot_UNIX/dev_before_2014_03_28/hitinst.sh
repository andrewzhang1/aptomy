...
(
# unset APBIN APCC APIPATH _tmp hittmp hit  \
# 	AP_AGENTPORT AP_AGTPLIST AP_CRRTSK AP_DEFAULT_CCC AP_DEFVAR AP_DEF_ACCPATH \
# 	AP_DEF_AGENTPORTLIST AP_DEF_BASPATH AP_DEF_BINPATH AP_DEF_DATPATH AP_DEF_DIFFOUT \
# 	AP_DEF_DMAPP AP_DEF_EMAILLIST AP_DEF_ENVPATH AP_DEF_ENVPATH_TEMPLATE \
# 	AP_DEF_FLGPATH AP_DEF_IGNORESH AP_DEF_INSTERRSTS AP_DEF_LICPATH AP_DEF_LOGPATH \
# 	AP_DEF_MONPATH AP_DEF_QTSK AP_DEF_RESLOC AP_DEF_RESPATH AP_DEF_RSPPATH \
# 	AP_DEF_SCRIPTLIST AP_DEF_SECPATH AP_DEF_SHABBR AP_DEF_SNAPSHOT_BASEPATH \
# 	AP_DEF_STSPATH AP_DEF_SXRAGENTPORTLIST AP_DEF_TMPPATH AP_DEF_TSKPATH \
# 	AP_DEF_VIEWPATH AP_NOPLAT AP_RTDIFFILE AP_RTENVFILE AP_RTFILE \
# 	AP_RTSOGFILE AP_RTSTAFILE AP_TSKDONE AP_TSKQUE AP_VERSEPCH AP_VERSION \
# 	AUTOPILOT BUILD_ROOT ESSBASEPATH ESSDIR ESSLANG HIT_ROOT \
# 	ODBCINI ODBCINST ODBC_HOME OHOME PREREL PROD_ROOT \
# 	SXR_HOME SXR_STA_ACCUMULATED VIEW_PATH _OPTION _VER_HITVER _apbin \
# 	_aporgps1 _apps1 _cmpsts _hitpref _hyperion_home _tallver \
# 	alllck alltsk ampar amsal aphis apsal arch bld dbg flag hit_inst_loc \
# 	hitloc keepzip my_crr_base mytsk nocleanup noclient node one \
# 	oneitem orgpar pathsep plat platlck platrlck plattsk stgfile \
# 	svusr _myver arborpath_backup LPATH this_flag_file this_status_file \
# 	thishost thiskernel thisnode thisplat thissvusr tsklck vbld ver wrtcrr zip
# ORACLE_HOME HYPEIRON_HOME JAVA_HOME
unset LPATH
# set > $HOME/.vars.txt
mh=${HYPERION_HOME%/*}
if [ -d "$mh" ]; then
...
if [ `uname` = "Windows_NT" -a -f "$ARBORPATH/bin/essgapinu.dll" ]; then
	workaround=$HYPERION_HOME/workaround.txt
...
if [ `uname` = "Windows_NT" -a -f "$AUTOPILOT/data/odbc${ODBC_HOME##*/}.reg" ]; then
	odbcreg.sh -reg < $AUTOPILOT/data/odbc${ODBC_HOME##*/}.reg
fi
exit $sts
