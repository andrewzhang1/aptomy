...
# 05/15/2013 YKono - BUG 15875316 - ADD MACHINE NAME TO DFL FILES
# 05/21/2013 YKono - BUG 15875316 - ADD MACHINE NAME TO DFL FILES with secmode
# 06/21/2013 YKono - Add workaround for when analyze.pl cause error and failed to create
#                    input difs.txt
...
# echo "dif=$1, bld=<$bld>, desc=<$desc>, comm=<$comm>"
	if [ -n "$desc" ]; then
		if [ "${desc%\(*}" = "$desc" ]; then
			...
			echo "$thisbld~$thisplt~$thisabv~$1~(${bld}:2)~$2~$LOGNAME~$thishost~$secmode" >> ../dfl/${thisver}.dfl
		else
			...
			echo "$thisbld~$thisplt~$thisabv~$1~(${bld}:${ccnt})~$2~$LOGNAME~$thishost~$secmode" >> ../dfl/${thisver}.dfl
		fi
	else
		if [ -n "$bld" ]; then
			...
			echo "$thisbld~$thisplt~$thisabv~$1~($thisbld:1)~$2~$LOGNAME~$thishost~$secmode" >> ../dfl/${thisver}.dfl
	...
	#    V
	# First dif:
	# <difname>     <date>(bld#:1)	# script list
	# Second dif:
	# <difname>  <date or desc>(bld#:2)   # script list
	# Third or more dif:
	# <difname>   <date or desc>(bld#:cc#)  # script list
}
...
# Decide archive file and extract it to some where
[ -f "$output" ] && rm -rf "$output"
touch $output
while read line shdata; do
	if [ "$shdata" = "#ANALYZE ERROR" ]; then
		anaerr="true"
	else
		if [ "$anaerr" = "true" ]; then
			echo "$thisbld~$thisplt~$thisabv~$line~(#NA:#NA)~#NA~$LOGNAME~$thishost~$secmode" >> ../dfl/${thisver}.dfl
		else
			[ "${line%${line#?}}" != "#" ] && edit_dif $line "$shdata"
		fi
	fi
done
