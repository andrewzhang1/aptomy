...
# 11/18/13 YK - Bug 17806229 - FILESIZE.PL ENTER INFINITY LOOP WHEN EXECUTED BY PERL 5.16.3
...
if [ "$plat" != "${plat#win}" ]; then
	mks=`which mksinfo`
	mks=${mks%/*}
	fspl=`which filesize.pl`
	${mks}/perl.exe ${fspl} -attr $_VER_DIRLST -ign "${_VER_DIRCMP}" -dyn "${_VER_DYNDIR}" >> "$bfile"
else
