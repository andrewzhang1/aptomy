...
apply_opack()
{
	for opdir in $opdirs; do
		...
		if [ "$whichOpack" = "rtc" -a "${thisplat#win}" != "$thisplat" ]; then
			Tasklist /m dbghelp.dll | grep -v "========" | grep -v "^Image" \
				| grep -v "^$" | egrep -i -v "tasklist.exe" | while read nm pid mod; do
				kill -9 $pid
			done
		fi
		if [ "$skipf" = "false" ]; then
			...
			if [ "$whichOpack" = "server" -a -f "$ESSBASEPATH/bin/essbase_sso.cfg" ]; then
				...
				# cat $ESSBASEPATH/bin/essbase_sso.cfg | while read line; do
				# 	echo "### > $line"
				# done
			...
			echo "y" > _tmpimp_
			echo "y" >> _tmpimp_
			echo "OPATCH FOLDER=$opdir."
			# Check pre-applied module exists
			## Get patch ID
			refid=`cat $opdir/etc/config/inventory 2> /dev/null | grep "reference_id number"`
			refid=${refid#*=\"}
			refid=${refid%\"*}
			...
			if [ $? -eq 0 ]; then # exists
				...
				#opackcmd.sh rollback -id $refid | while read line; do
				#	echo "  > $line"
				#done
			fi
			# Apply opack
			...
			if [ -f "$HOME/.essbase_sso.cfg.saved" ]; then
				[ -f "$ESSBASEPATH/bin/essbase_sso.cfg" ] && mv $ESSBASEPATH/bin/essbase_sso.cfg $ESSBASEPATH/bin/essbase_sso.cfg.replaced.by.opatch
