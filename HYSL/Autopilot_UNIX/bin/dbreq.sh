...
display_help()
{
	echo "dbreq.sh : Request to create/initialize the user"
	echo "Syntax: dbreq.sh [<trg-host>] [-h|-n <user>]"
	echo " <trg-host> : Target host name. When you skip this parameter,"
	echo "              Any dbd.sh may receive your request."
	echo " -h         : Display this."
	echo " -n <user>  : The user name to create/initialize."
	echo "              When you skip this parameter, dbreq.sh will uses"
	echo "              current user and host name($accnt)."
}
