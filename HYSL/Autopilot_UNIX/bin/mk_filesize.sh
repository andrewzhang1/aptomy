...
# Get search location
if [ -z "$dirlist" ]; then
	if [ -f "$AP_DEF_DATPATH/dircmp_pattern.txt" ]; then
		dirlist=`tail -1 "$AP_DEF_DATPATH/dircmp_pattern.txt"`
	else
		dirlist="$_VER_DIRLST"
	fi
fi
...
elif [ -f "$AP_DEF_DATPATH/dircmp_ignore_${LOGNAME}@${host}.txt" ]; then
	AP_DIRCMP_IGNORE=`tail "$AP_DEF_DATPATH/dircmp_ignore_${LOGNAME}@${host}.txt"`
	AP_DIRCMP_IGNORE=${AP_DIRCMP_IGNORE%AP_DIRCMP_IGNORE=}
elif [ -f "$AP_DEF_DATPATH/dircmp_ignore_${plat}.txt" ]; then
	AP_DIRCMP_IGNORE=`tail -1 "$AP_DEF_DATPATH/dircmp_ignore_${plat}.txt"`
	AP_DIRCMP_IGNORE=${AP_DIRCMP_IGNORE%AP_DIRCMP_IGNORE=}
elif [ -f "$AP_DEF_DATPATH/dircmp_ignore.txt" ]; then
	AP_DIRCMP_IGNORE=`tail -1 "$AP_DEF_DATPATH/dircmp_ignore.txt"`
	AP_DIRCMP_IGNORE=${AP_DIRCMP_IGNORE%AP_DIRCMP_IGNORE=}
else
