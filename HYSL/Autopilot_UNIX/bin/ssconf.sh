...
if [ `cmpstr $iver 011001002002000` != "<" ]; then
	# All ports
	calcport="|epm_port|cmpct_port|cmpct_sport|ss_port|ss_sport|ohs_port|ohs_sport|ssl_port|agent_port|start_port|end_port|"
else
	# Essbase only
...
if [ -n "$_fail" ]; then
	...
	echo "# Stop HSS($edprg) and Essbase related serivces."
