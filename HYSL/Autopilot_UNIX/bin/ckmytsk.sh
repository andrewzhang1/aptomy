...
chk_execplat()
{
	...
	while [ -n "$ignplats" ]; do
		...
		case $oneplat in
			$LOGNAME|$thishost|$thisnode|$thisplat)
				ret=ign
				break
				;;
			*)
	...
	if [ "$ret" = "true" ]; then
		...
		if [ -n "$freemem" ]; then
			case $freemem in
				*KB)    freemem=${freemem%??};;
				*MB)    let freemem=${freemem%??}\*1024 2> /dev/null;;
				*GB)    let freemem=${freemem%??}\*1048576 2> /dev/null;;
				*B)     let freemem=\(${freemem%?}+512\)/1024 2> /dev/null;;
				*)	;;
			esac
			crrmem=`get_freemem.sh`
...
if [ "$nodelck" = "true" ]; then
	unlock.sh $AUTOPILOT/lck/${thisnode}.read > /dev/null 2>&1
fi
if [ "$platlck" = "true" ]; then
	unlock.sh $AUTOPILOT/lck/${thisplat}.read > /dev/null 2>&1
fi
