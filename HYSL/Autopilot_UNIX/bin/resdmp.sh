...
for plat in $plats; do
	...
	tf=
	if [ -z "$tskf" ]; then
		if [ "$AP_NOPLAT" = "false" -a -f "$AUTOPILOT/tsk/${plat}.tsk" ]; then
			[ "$dbg" = "true" ] && echo "# Check $AUTOPILOT/tsk/${plat}.tsk"
			a=`grep "^${ver}" $AUTOPILOT/tsk/${plat}.tsk`
			if [ -n "$a" ]; then
				[ "$dbg" = "true" ] && echo "# - Found ${ver} in ${plat}.tsk"
				grep "^${ver}" $AUTOPILOT/tsk/${plat}.tsk | grep -v "^#" \
		...
		[ "${tskf#*/}" = "$tskf" ] && tskf="$AUTOPILOT/tsk/$tskf"
		[ -f "$tskf" ] && tf=$tskf
		[ "$dbg" = "true" ] && echo "# > tskf=$tskf"
	fi
	if [ -z "$tf" ]; then
