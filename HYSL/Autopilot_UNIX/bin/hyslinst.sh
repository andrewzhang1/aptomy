...
if [ "$instkind" = "bi" ]; then
	...
	if [ -n "$opbase" ]; then
		hh=`. se.sh $ver -nomkdir > /dev/null 2>&1; echo $HYPERION_HOME`
		export AP_BISHIPHOME=true
		[ -z "$AP_SECMODE" ] && export AP_SECMODE=rep
		echo "Use \"biinst.sh $opbase ${hh%/*}\" command."
		biinst.sh $opbase ${hh%/*}
		sts=$?
		if [ "$sts" -eq 0 ]; then
	...
	while [ 1 ]; do
		...
		(. se.sh $ver > /dev/null 2>&1; hitinst.sh $rspf $use_zip $opbase hit[${hitbld}])
		sts=$?
		if [ "$sts" -ne 0 ]; then
	...
	if [ "$sts" -eq 0 ]; then
		(	. se.sh $ver > /dev/null 2>&1
			...
			if [ "$crressv" != "$ver:$ebld" ]; then
				...
				sts=$?
				if [ "$sts" -eq 0 ]; then
					echo "$ver:$ebld(base:$opbase hit[${hitbld}])" > $HYPERION_HOME/opack_version.txt
				else
...
if [ "$sts" -eq 0 ]; then
	...
	(. se.sh $ver > /dev/null 2>&1; chk_essinst.sh $ver;sts=$?;echo "# chk_essinit.sh sts=$sts"; return $sts)
	if [ $? -eq 0 ]; then
		[ "$keep_prev_app" = "true" ] && (. se.sh $ver > /dev/null; restore_prev_app)
		exit 0
	else
...
case "`get_platform.sh`" in
	aix|solaris|linux)	echo "32" | refresh ${_AP_REFRESHOPT} ${mver}:${sver}${vbld} ;;
	win32|hpux|hpux64)	refresh ${_AP_REFRESHOPT} ${mver}:${sver}${vbld} ;;
