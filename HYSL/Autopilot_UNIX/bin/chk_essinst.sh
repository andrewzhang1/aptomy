...
if [ "$AP_SECMODE" = "rep" -o "$AP_SECMODE" = "fa" ]; then
	...
	# # Start Web logic
	# if [ "`uname`" = "Windows_NT" ]; then
	# 	${HYPERION_HOME%/*}/user_projects/domains/bifoundation_domain/bin/startWebLogic.bat
	# else
	# 	${HYPERION_HOME%/*}/user_projects/domains/bifoundation_domain/bin/startWebLogic.sh
	# fi
