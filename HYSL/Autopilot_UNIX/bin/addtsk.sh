...
if [ -z "$tst" ]; then
	if [ -n "$tskf" ]; then
		[ "${tskf#*/}" = "$tskf" ] && \
			tskf=$AUTOPILOT/tsk/$tskf
		tskfkind="list"
	else
		if [ "$platkind" = "node" ]; then
			tskf=$AUTOPILOT/tsk/${plat%@*}.tsk
			tskfkind="personal"
		else
			tskf=$AUTOPILOT/tsk/${plat}.tsk
			tskfkind="platform"
...
if [ -z "$tst" ]; then
	...
	if [ "$tskfkind" = "personal" ]; then
		cat $tskf 2> /dev/null | grep "^${plat#*@}:${srchver}" | sed -e "s/^[^:]*://g" | \
			sed -e "s/^\(${ver}:\)[^:]*:/\1${bld}:/g" | tr -d '\r' > $tsktmp
	elif [ "$tskfkind" = "platform" ]; then
		cat $tskf | grep "^${srchver}" | sed -e "s/^\(${ver}:\)[^:]*:/\1${bld}:/g" | tr -d '\r' > $tsktmp
	else
		fromctl=$(
