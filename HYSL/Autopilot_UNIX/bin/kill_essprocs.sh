...
killhssbi()
{
	if [ "`uname`" = "Windows_NT" ]; then
		if [ $# -ne 0  ]; then
			pstree.pl -all | sed -e "s!\\\!/!g" | grep "<none>" | \
			...
			pstree.pl -all | sed -e "s!\\\!/!g" | grep "<none>" | \
				egrep "nmz.exe|opmn.exe|startEPMServer|weblogic_patch|startNodeManager|startWebLogic|nqsclustercontroller|nsqserver|nsqscheduler|sawserver" \
