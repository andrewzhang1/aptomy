...
wrtwsp2()
{
	...
	if [ $# -ne 0 ]; then
		...
		if [ -f "$AUTOPILOT/instlog/${LOGNAME}@${myhost}~${VERSION}~${BUILD}.log" ]; then
			echo "" >> $_rtfname
			echo "### Installation log ($AUTOPILOT/instlog/${LOGNAME}@${myhost}~${VERSION}~${BUILD}.log)" >> $_rtfname
			(IFS=
				cat "$AUTOPILOT/instlog/${LOGNAME}@${myhost}~${VERSION}~${BUILD}.log" 2> /dev/null | while read line; do
					echo "  $line" >> $_rtfname
				done
			)
		fi
...
echo "# _ishit=$_ishit"
		...
		if [ "$_force" = "true" ]; then
			...
			echo "$thisnode($thisplat):`(. se.sh $VERSION; get_instkind.sh -l)`" >> \
				$AUTOPILOT/instlog/${LOGNAME}@$(hostname)~${VERSION}~${BUILD}.log
...
if [ "$verbose" = "true" ]; then
	...
	if [ $difcnt -ne 0 ]; then
		...
		if [ -f "${TEST%.*}.sta" ]; then
			...
			cat $EMAIL_LIST 2> /dev/null | grep "^${_cdn_label_}[0-9]*[ 	]"  \
...
# echo "$difname	$difbld	$difnum[$difdesc]	$shlist"
					...
					if [ "$_putpdif" = "true" -a -z "$_ignorable" ]; then
						for _owner in $_owners; do
							...
							_owtmp=`echo $_owner | sed -e s!\<!~!g -e s!\>!~!g -e s!\@!~!g`
							msgfile=$AP_DEF_LOGPATH/${_owtmp}_${mysvnode}_${VERSION##*/}_${BUILD}_${TEST_ABV}.msg
							if [ ! -f "$msgfile" ]; then
								if [ -n "$_ct" ]; then
