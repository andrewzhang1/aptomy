...
#
#  8/27/2010 Yukio Kono
#            Use /usr/ucb/ps command for Solaris platform.
#

case "$SXR_PLATFORM" in
   solarisx86 | solaris | hpux | aix | linux ) path_sep=":" ;;
...
then
   ...
   case "$SXR_PLATFORM" in
      solarisx86 | solaris ) LD_LIBRARY_PATH="${SXR_ALTARBORPATH}/bin${path_sep}${LD_LIBRARY_PATH}" ;;
...
# Check AUTOPILOT framework is ready or not -> _AP_READY
_AP_READY="false"
_apready=`which apinc.sh`
if ( test $? -eq 0 -a "x${_apready#no}" = "x${_apready}" )
then
   _AP_READY="true"
fi

# Check the client is working with server service deamon
if ( test "$_AP_READY" = "true" )
then
   _cl_lckf="${AUTOPILOT}/lck/${LOGNAME}@$(hostname).cl"
   if ( test -f "$_cl_lckf.lck" )
   then
      lock_ping.sh $_cl_lckf > /dev/null 2>&1
      if ( test $? -ne 0 )
      then
         _AP_READY="false"
      else
         _ap_cpid=`cat $_cl_lckf.lck | grep "target=" | crfilter`
         _ap_cpid=${_ap_cpid#*target=}
         if ( test -z "$_ap_cpid" )
         then
            _AP_READY="false"
         fi
...
# If autopilot framework is ready, we can manipulate remote server via autopilot.
# Otherwise, we need to give up when the server is remote.
if ( test "$_AP_READY" != "true" )
then
...
case $flag in
  ...
  start)
    ...
    if ( test "$_AP_READY" = "true" )
    then
        svcmd.sh -pid $_ap_cpid start_essbase.sh
    else
        if ( test "$SXR_MODE" = "1")
        then
          case $SXR_PLATFORM in
            nti|nta)
              $HYPERION_HOME/../user_projects/epmsystem1/bin/startEssbase.bat
            ;;
            *)
              $HYPERION_HOME/../user_projects/epmsystem1/bin/startEssbase.sh
            ;;
          esac
        else
  ...
  stop)
    ...
    then
      ...
      then
        if ( test "$_AP_READY" = "true" )
        then
            svcmd.sh -pid $_ap_cpid stop_essbase.sh
        else
            case $SXR_PLATFORM in
               nti|nta)
                 $HYPERION_HOME/../user_projects/epmsystem1/bin/stopEssbase.bat
               ;;
               *)
                 $HYPERION_HOME/../user_projects/epmsystem1/bin/stopEssbase.sh
               ;;
             esac
        fi
      else
  ...
  kill)
    ...
    if ( test "$_AP_READY" = "true" )
    then
        svcmd.sh -pid $_ap_cpid "kill_essprocs.sh -all"
    else
        for pid in $(eval ${PID})
        do
          kill -9 $pid
